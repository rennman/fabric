...
if (tutil.tlsOn) {
   ...
   if (tutil.peerAddr0.match('zone.blockchain.ibm.com')) deployCert = '/root/certificate.pem'
   else if (tutil.peerAddr0.match('us.blockchain.ibm.com')) deployCert = '/certs/blockchain-cert.pem'
...
function devMode(c) {
   ...
   //console.log("\nOn entry:  initialDevMode", initialDevMode);
   c.setDevMode(c.isDevMode() ? false : true);  /* toggle boolean */
   //console.log("next DevMode", c.isDevMode());
   ...
   c.setDevMode(c.isDevMode() ? false : true);  /* toggle boolean */
   //console.log("final DevMode", c.isDevMode())
   if (c.isDevMode() != initialDevMode)  {
...
function invokeWaitChallenge(cb) {
   ...
   //console.log("InvokeWaitTime", chain.getInvokeWaitTime());
...
test('Invoke a chaincode by enrolled user', function (t) {
    ...
    //chain.setInvokeWaitTime("invoke wait time");
    // Trigger the invoke transaction
    var invokeTx = test_user_Member1.invoke(invokeRequest);
