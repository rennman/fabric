...
function deploy(cb) {
    if (tutil.tlsOn) {
       ...
       if (peerAddr0.match('zone.blockchain.ibm.com')) deployCert = '/root/certificate.pem'
       else if (peerAddr0.match('us.blockchain.ibm.com')) deployCert = '/certs/blockchain-cert.pem'
...
test('not assign asset management with roles', function (t) {
    ...
    bob.getUserCert(["role", "account"], function (err, bobCert) {
        if (err) fail(t, "Failed getting Application certificate for Bob.");
